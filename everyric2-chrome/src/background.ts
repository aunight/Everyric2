...
async function getServerConfig(): Promise<ServerConfig> {
  const { serverUrl, apiKey, translationApiKey } = await getSettings();
  return {
    serverUrl,
    apiKey,
    translatorApiKey: selectTranslationApiKey(serverUrl, translationApiKey),
  };
}
...
async function handleMessage(message: BgRequest): Promise<MessageResponse> {
  switch (message.type) {
    ...
    case 'FETCH_NETEASE': {
      const match = await fetchFromNetease(message.payload);
      const { translationLanguage } = await getSettings();
      const data = match ? neteaseToLyricsData(match.lyric, translationLanguage) : null;
      if (data && match) {
        data.attribution = {
          name: 'NetEase Cloud Music',
          url: `https://music.163.com/#/song?id=${match.track.id}`,
          sourceId: 'netease',
        };
      }
      return { data };
    ...
    case 'PICK_NETEASE': {
      const lyric = await fetchNeteaseLyric(message.payload.id);
      const { translationLanguage } = await getSettings();
      return { data: lyric ? neteaseToLyricsData(lyric, translationLanguage) : null };
    }
...
async function searchCandidates(query: { title: string; artist: string; duration: number }): Promise<SearchCandidate[]> {
  const [tracks, neteaseTracks, wikiMatch] = await Promise.all([
