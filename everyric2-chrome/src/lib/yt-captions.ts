...
/** 표시용 언어 이름 (예: "ja" → 표시 언어에 따라 "일본어"/"日文"). 실패하면 유튜브 라벨.
 *  로케일은 uiLanguage를 따른다 — 예전엔 'ko' 고정이라 zh/en/ja UI에 한국어가 샜다. */
export function languageDisplayName(track: YtCaptionTrack, uiLang: string): string {
  const locale = uiLang === 'zh' ? 'zh-TW' : uiLang || 'en';
  try {
    const name = new Intl.DisplayNames([locale], { type: 'language' }).of(track.lang);
    if (name && name !== track.lang) return name;
  } catch {
    /* Intl.DisplayNames 미지원/미지 로케일 — 라벨로 폴백 */
  }
  return track.label || track.lang;
}

/** 출처 배지에 붙일 문구 (예: "日文 · 上傳者字幕") */
export function captionSourceLabel(track: YtCaptionTrack, uiLang: string): string {
  const kind = track.auto ? t('captions.autoGenerated') : t('captions.uploader');
  return `${languageDisplayName(track, uiLang)} · ${kind}`;
}
