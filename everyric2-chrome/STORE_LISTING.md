...
- [ ] **Cloudflare Bot Fight Mode에서 `/privacy`·`/health`를 예외로 뺄 것 — 사용자가
      대시보드에서 해야 한다.** 지금 게이트웨이가 **User-Agent로 요청을 거른다**(실측
      2026-07-26): `Python-urllib/3.12`은 `/privacy`조차 **403**이고, curl·브라우저·
      Googlebot·UA 없음은 모두 200이다. 심사자가 브라우저로 열면 통과하지만 **자동 검사가
      어떤 UA를 쓰는지 우리는 모르고, 방침 URL 도달 불가는 반려 사유로 직결된다.**
      조치: Configuration Rule로 그 두 경로만 BIC 해제, `/api/*`는 유지(스크래퍼 차단은
      그대로 필요하다). 정적 HTML이고 읽히는 것이 목적이므로 해제 비용은 없다.
      — 이 규칙은 서버 쪽 soak 러너가 `127.0.0.1` 직결이라 Cloudflare를 거치지 않아
      발견되지 않았다. **공개 경로는 반드시 공개 URL로 검증해야 한다.**
