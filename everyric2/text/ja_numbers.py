...
이 모듈은 숫자 자체의 읽기만 만든다. **조수사(秒 등) 앞에서만** 호출하는 결정은
``ja_reading``이 한다 — 조수사에 따라 촉음화·반탁음화(一分→いっぷん)가 걸리는데, 그
음변화표는 실측된 조수사만 다룬다(현재 秒 하나, 秒는 음변화가 없는 규칙 조수사라
이 모듈의 자릿수 읽기를 그대로 이어 붙이면 된다). 이 모듈 자체는 조수사를 모른다 —
숫자 자릿수 읽기는 조수사와 무관하게 고정된 문법(백/천의 촉음·반탁음화 포함)이라
"실측 없이 만들지 마라" 원칙의 대상이 아니다.
"""
