...
패널에 커팅 섹션. 글자를 `span`으로 렌더하고 사이마다 클릭 타깃(캐럿)을 둔다. 각 글자 아래에
그 글자의 시각을, 라인 아래에 발음을 **참고용으로** 표시한다 — 일본어는 어디서 끊을지가 바로
그 정보에 달려 있다. 컷 표시된 캐럿을 다시 누르면 되붙이고, 컷 경계는 좌우 드래그로 미세조정한다.
